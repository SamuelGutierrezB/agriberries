...
public class UpdateUnitActivity extends AppCompatActivity {
    ...
    private int previousCropPosition;
    ...
    private void setup() {
        ...
        Button btnUpdateUnit = findViewById(R.id.btnUpdateUnit);

        // Get unit and initialize coordinates
        ...
        ArrayAdapter<CharSequence> adapterState = ArrayAdapter.createFromResource(UpdateUnitActivity.this, R.array.mexican_states, android.R.layout.simple_spinner_item);
        adapterState.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        ArrayAdapter<String> adapterCrop = new ArrayAdapter<>(UpdateUnitActivity.this, android.R.layout.simple_spinner_item, cropNameList);
        adapterCrop.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        ArrayAdapter<CharSequence> adapterSoil = ArrayAdapter.createFromResource(UpdateUnitActivity.this, R.array.soil, android.R.layout.simple_spinner_item);
        adapterSoil.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        ArrayAdapter<CharSequence> adapterManagement = ArrayAdapter.createFromResource(UpdateUnitActivity.this, R.array.management, android.R.layout.simple_spinner_item);
        adapterManagement.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        ArrayAdapter<CharSequence> adapterModality = ArrayAdapter.createFromResource(UpdateUnitActivity.this, R.array.modality, android.R.layout.simple_spinner_item);
        adapterModality.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);

        // Initialize variables
        previousCropPosition = adapterCrop.getPosition(unit.getCrop());

        // Initialize views
        String coordinates = unit.getLatitude() + ", " + unit.getLongitude();
        etvName.setText(unit.getName());
        etvLocation.setText(unit.getLocation());
        etvAltitude.setText(String.valueOf(unit.getAltitude()));
        if (latitude != -1000 && longitude != -1000) tvCoordinates.setText(coordinates);
        etvHectares.setText(String.valueOf(unit.getHectares()));
        spinnerState.setAdapter(adapterState);
        spinnerState.setSelection(adapterState.getPosition(unit.getState()));
        spinnerCrop.setAdapter(adapterCrop);
        spinnerCrop.setSelection(adapterCrop.getPosition(unit.getCrop()));
        spinnerSoil.setAdapter(adapterSoil);
        spinnerSoil.setSelection(adapterSoil.getPosition(unit.getSoil()));
        spinnerManagement.setAdapter(adapterManagement);
        spinnerManagement.setSelection(adapterManagement.getPosition(unit.getManagement()));
        spinnerModality.setAdapter(adapterModality);
        spinnerModality.setSelection(adapterModality.getPosition(unit.getModality()));

        // Initialize variables
        String clientId = getIntent().getStringExtra("clientId");
        FirebaseFirestore db = FirebaseFirestore.getInstance();
        unitDocumentReference = db.collection("clients").document(clientId)
                .collection("units").document(unit.getId());

        // Get blocks
        CollectionReference collectionReference = unitDocumentReference.collection("blocks");
        collectionReference.get().addOnSuccessListener(queryDocumentSnapshots -> {
            // Copy all blocks to the Array List
            blocks.addAll(queryDocumentSnapshots.toObjects(Block.class));
            ...
            for (int row = 0; row < rows; row++) {
                for (int col = 0; col < cols; col++) {
                    ...
                    gridButtons[row][col] = new Button(UpdateUnitActivity.this);
                    ...
                    gridButtons[row][col].setOnClickListener(v -> {
                        ...
                        AlertDialog.Builder builder = new AlertDialog.Builder(UpdateUnitActivity.this, R.style.CustomAlertDialog);
                        ...
                        // Create type adapter
                        ArrayAdapter<String> adapterType = new ArrayAdapter<>(UpdateUnitActivity.this, android.R.layout.simple_spinner_item, cropList.get(spinnerCrop.getSelectedItemPosition()).getTypes());
                        ...
                        linearLayoutPlantation.setOnClickListener(v1 -> {
                            ...
                            DatePickerDialog datePickerDialog = new DatePickerDialog(UpdateUnitActivity.this, (view, year, month, dayOfMonth) -> {
                        ...
                        btnSaveBlock.setOnClickListener(v1 -> {
                            ...
                            if (!name.isEmpty() && !furrowDistanceText.isEmpty() && !plantDistanceText.isEmpty()) {
                                ...
                                if (calendarHarvest.compareTo(calendarPlantation) > 0) {
                                    ...
                                    boolean used = false;
                                    for (Block block : blocks) {
                                        if (block.getName().equals(name) && !gridButtons[finalRow][finalCol].getText().toString().equals(block.getName())) {
                                            used = true;
                                            break;
                                        }
                                    }
                                    ...
                                    if (!used) {
                                        ...
                                        if (usedBlock != null) {
                                            // Update block
                                            usedBlock.setName(name);
                                            usedBlock.setFurrowDistance(Double.parseDouble(furrowDistanceText));
                                            usedBlock.setPlantDistance(Double.parseDouble(plantDistanceText));
                                            usedBlock.setType(type);
                                            usedBlock.setRow(finalRow);
                                            usedBlock.setCol(finalCol);
                                            usedBlock.setPlantationDate(calendarPlantation.getTime());
                                            usedBlock.setHarvestDate(calendarHarvest.getTime());
                                        } else {
                                            // Create new block
                                        ...
                                        // Inform user
                                        Toast.makeText(UpdateUnitActivity.this, getResources().getString(R.string.blockSaved), Toast.LENGTH_SHORT).show();
                                        dialogBlock.dismiss();
                                    } else Toast.makeText(UpdateUnitActivity.this, getResources().getString(R.string.block_name_already_used), Toast.LENGTH_SHORT).show();
                                } else Toast.makeText(UpdateUnitActivity.this, getResources().getString(R.string.second_date_must_be_greater), Toast.LENGTH_SHORT).show();
                            } else Toast.makeText(UpdateUnitActivity.this, getResources().getString(R.string.unfilled), Toast.LENGTH_SHORT).show();
                        });
                        ...
                        btnDeleteBlock.setOnClickListener(v2 -> {
                            ...
                            Toast.makeText(UpdateUnitActivity.this, getResources().getString(R.string.blockDeleted), Toast.LENGTH_SHORT).show();
        ...
        }).addOnFailureListener(e -> Toast.makeText(UpdateUnitActivity.this, getResources().getString(R.string.serverFailed), Toast.LENGTH_SHORT).show());
        ...
        spinnerCrop.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if (blocks.size() > 0 && position != previousCropPosition) {
                    ...
                    LayoutInflater inflater = LayoutInflater.from(UpdateUnitActivity.this);
        ...
        btnUpdateUnit.setOnClickListener(v -> updateUnit());
    }
    ...
    private void updateUnit() {
        ...
        if (!name.isEmpty() && !location.isEmpty() && !textAltitude.isEmpty() && !textHectares.isEmpty() && !blocks.isEmpty()) {
            ...
            // Delete all previous blocks
            for (Block previousBlock : previousBlocks) {
                if (!blocks.contains(previousBlock)) {
                    // Get document reference and add it to the batch
                    DocumentReference documentReference = unitDocumentReference
                            .collection("blocks").document(previousBlock.getId());
            ...
            batch.commit();
            Toast.makeText(UpdateUnitActivity.this, getResources().getString(R.string.unitUpdated), Toast.LENGTH_SHORT).show();
            finish();
